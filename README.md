...
```
And for `(D, allowance) = (18, 13), (24, 15), (36, 19)`. The last one is also the `tx_depth` requirement for a checkpoint candidate.
```golang
...
`x` is at least 36. Looks fine already, and even more so considering that the fast block generation does not happen a lot.
...
We will set `L` to 11, and then `x` is at least 35, which is good enough (the actual value of `L`is 11.1 minutes, or 666 seconds).
